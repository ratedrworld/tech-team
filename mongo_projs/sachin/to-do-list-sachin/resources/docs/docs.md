...
The `home-routes` handler in the `to-do-list-sachin.routes.home` namespace
...
The `home-page` function will in turn call the `to-do-list-sachin.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `to-do-list-sachin.handler` namespace:
...
Request middleware functions are located under the `to-do-list-sachin.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `to-do-list-sachin.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `to-do-list-sachin.core` in some other namespace.
