...
The `home-routes` handler in the `ajax-to-do.routes.home` namespace
...
The `home-page` function will in turn call the `ajax-to-do.layout/render` function
...
The rest of this page is rendered by ClojureScript found in the `src/cljs/ajax_to_do/core.cljs` file.



#### Organizing the routes

The routes are aggregated and wrapped with middleware in the `ajax-to-do.handler` namespace:
...
Request middleware functions are located under the `ajax-to-do.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `ajax-to-do.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `ajax-to-do.core` in some other namespace.
